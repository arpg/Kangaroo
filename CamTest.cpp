...
{
//    // Check if strobe functionality is supported
//    dc1394error_t e;
//    uint32_t strobe_inq;
...
void ClearDmaBuffer(dc1394camera_t* cam)
{
    dc1394video_frame_t* frame;
    dc1394_capture_dequeue(cam, DC1394_CAPTURE_POLICY_POLL, &frame);
...
public:
    ...
    int Init()
    {
        ...
        SetAbsShutterTime(m_pCam[0], 1.0f/120.0f);
        SetAbsShutterTime(m_pCam[1], 1.0f/120.0f);

        // Setup Triggering
        if(false) {
            DisableTrigger(m_pCam[0]);
            DisableTrigger(m_pCam[1]);
        }else{
            SetTriggerCam1FromCam2(m_pCam[0], m_pCam[1]);
        }
...
//        // Clear DMA buffers to sync cameras to start
